...
Usando as funções `read_state()` e `read_region()` do pacote `geobr` é possível obter as coordenadas de todos os estados e regiões do Brasil.
...
Em seguida, para garantir uma boa visualização das frequências, as teses foram agrupadas por estado e unidas com as coordenadas extraídas do pacote `geobr`.
...
É possível observar que São Paulo (SP) é de longe o estado que mais produz na seleção que fiz.Seguido de Rio de Janeiro (RJ) e Ceará (CE)


### Mapa da frequência de trabalhos por região

Utilizando a mesma lógica do mapa anterior, construí o mapa abaixo contando o número de trabalhos defendidos entre as regiões do Brasil.
...
ggplot(teses_por_regiao, aes(fill = total_trabalhos_regiao, label =  paste(regiao, " (", total_trabalhos_regiao, ")", sep = "")))+
  geom_sf()+
...
Para o cálculo, utilizei a função `case_when()` para filtrar os níveis (mestrado- doutorado) e contabilizar o número de trabalhos totais.
...
trabalhos_por_programa |>
  gt() |>
  tab_header(title = "Estatísicas descritivas de teses e dissertações de programas de pós-graduação 1987-2022") |>
